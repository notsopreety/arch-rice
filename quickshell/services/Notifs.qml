...
Singleton {
    ...
    signal notificationAdded(var notifWrapper)
    ...
    function addNotification(notif) {
        // Enable tracking on the notification so replacement and updates are handled in-place
        notif.tracked = true
        ...
        // Check if an active notification with the same appName and summary already exists to replace it in place
        let existingWrapper = null
        const currentNotifs = root.notifications
        for (let i = 0; i < currentNotifs.length; i++) {
            if (currentNotifs[i] && 
                currentNotifs[i].appName === notif.appName && 
                (notif.appName === "Battery" || currentNotifs[i].summary === notif.summary) && 
                !currentNotifs[i].closed) {
                existingWrapper = currentNotifs[i]
                break
            }
        }

        if (existingWrapper) {
            QsServices.Logger.debug("Notifs", `Replacing existing notification in place: ${notif.summary}`)
            existingWrapper.notification = notif
            root.notificationAdded(existingWrapper)
            return
        }
        ...
        root.notificationAdded(notifWrapper)
    }
