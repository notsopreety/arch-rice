...
Singleton {
    ...
    property real cachedMemoryMB: 0
    property real buffersMemoryMB: 0
    ...
    readonly property bool isFullscreen: Hypr.activeToplevel?.fullscreen ?? false
    ...
    Process {
        id: dgopProcess
        command: ["python3", "/home/sawmer/.config/quickshell/scripts/sysinfo.py", root.activeModules]
                ...
                Qt.callLater(() => {
                    try {
                        ...
                        if (data.cpu) {
                            ...
                            root.cpuCoresUsages = data.cpu.cores || [];
                            root.cpuFrequency = data.cpu.frequency || "";
                            root.cpuGovernor = data.cpu.governor || "";
                            root.cpuArchitecture = data.cpu.architecture || "";
                        }

                        if (data.memory) {
                            root.memUsage = (data.memory.usedPercent || 0) / 100;
                            root.totalMemoryMB = Math.round((data.memory.total || 0) / 1048576);
                            root.usedMemoryMB = Math.round((data.memory.used || (data.memory.total - data.memory.available) || 0) / 1048576);
                            root.freeMemoryMB = Math.round((data.memory.free || 0) / 1048576);
                            root.cachedMemoryMB = Math.round((data.memory.cached || 0) / 1048576);
                            root.buffersMemoryMB = Math.round((data.memory.buffers || 0) / 1048576);
                            ...
                            root.totalSwapMB = Math.round(totalSwap / 1048576);
                            root.usedSwapMB = Math.round((totalSwap - freeSwap) / 1048576);
                        ...
                        if (data.diskmounts && Array.isArray(data.diskmounts)) {
                            let newStats = [];
                            data.diskmounts.forEach(disk => {
                                const path = disk.mount || disk.mountpoint || "";
                                // Filter out noise virtual filesystems common on Linux
                                if (path === "" || 
                                ...
                                // Determine a display label based on mount path
                                let displayLabel = path === "/" ? "Root (System)" : path.substring(path.lastIndexOf('/') + 1);
                                if (displayLabel === "") displayLabel = path;

                                newStats.push({
                                    path: path,
                                    label: displayLabel.toUpperCase(),
                                    hasAlias: true,
                                    usage: isNaN(pctValue) ? 0 : pctValue / 100,
                                    total: Math.round((disk.total_bytes || 0) / (1024 * 1024)) || 0,
                                    used: Math.round((disk.used_bytes || 0) / (1024 * 1024)) || 0
                                });
                            });
                            // Sort so Root/System is always first, then sort alphabetically by mount path
                            newStats.sort((a, b) => {
                                if (a.path === "/") return -1;
                                if (b.path === "/") return 1;
                        ...
                        if (data.gpu && (data.gpu.gpus || Array.isArray(data.gpu))) {
                            ...
                            root.availableGpus = gpus.map(gpu => ({
                                ...
                                typeLabel: gpu.typeLabel || "Unknown",
                                driver: gpu.driver || "",
                                usage: gpu.usage || 0
                            }));
                            root.gpuHistory = root.addToHistory(root.gpuHistory, gpus.length > 0 ? (gpus[0].usage || 0) : 0);
                        } else if (!isMonitorActive) {
