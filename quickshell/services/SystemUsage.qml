...
Singleton {
    ...
    property var cpuHistory: [] // Array of numbers (percentages) for graphing
    property var gpuHistory: [] // Array of numbers (percentages) for graphing
    ...
    Component.onCompleted: {
        ...
        // Initial static CPU details detection
    ...
    Process {
        id: cpuDetailsProc
        command: ["/bin/sh", "-c", "lscpu | grep -E 'Model name:|Core\\(s\\) per socket:|Thread\\(s\\) per core:|Socket\\(s\\):'"]
        running: false
        stdout: SplitParser {
            property int sockets: 1
            property int coresPerSocket: 1
            property int threadsPerCore: 1
            onRead: data => {
                var clean = data.trim()
                if (clean.startsWith("Model name:")) {
                    root.cpuModel = clean.replace("Model name:", "").trim()
                } else if (clean.startsWith("Core(s) per socket:")) {
                    coresPerSocket = parseInt(clean.replace("Core(s) per socket:", "").trim()) || 1
                } else if (clean.startsWith("Thread(s) per core:")) {
                    threadsPerCore = parseInt(clean.replace("Thread(s) per core:", "").trim()) || 1
                } else if (clean.startsWith("Socket(s):")) {
                    sockets = parseInt(clean.replace("Socket(s):", "").trim()) || 1
                }
                root.cpuCores = sockets * coresPerSocket
                root.cpuThreads = root.cpuCores * threadsPerCore
            }
        }
    }

    // CPU Live Frequency Update
    Process {
        id: cpuFreqProc
        command: ["/bin/sh", "-c", "grep -i 'cpu MHz' /proc/cpuinfo | head -1 | awk '{print $4}'"]
        running: false
        stdout: SplitParser {
            onRead: data => {
                var mhz = parseFloat(data.trim()) || 0
                if (mhz >= 1000) {
                    root.cpuFreq = (mhz / 1000.0).toFixed(2) + " GHz"
                } else {
                    root.cpuFreq = Math.round(mhz) + " MHz"
                }
            }
        }
    }
    ...
    Process {
        id: cpuTempProc
        command: ["/bin/sh", "-c", "pkg_temp_file=\"\"; for h in /sys/devices/platform/coretemp.0/hwmon/hwmon*; do if [ -d \"$h\" ]; then for f in \"$h\"/temp*_label; do if [ -f \"$f\" ]; then lbl=$(cat \"$f\"); if [ \"$lbl\" = \"Package id 0\" ] || [[ \"$lbl\" =~ [Pp]ackage ]]; then pkg_temp_file=\"${f%_label}_input\"; break 2; fi; fi; done; fi; done; if [ -n \"$pkg_temp_file\" ] && [ -f \"$pkg_temp_file\" ]; then cat \"$pkg_temp_file\"; else temp_file=\"/sys/class/thermal/thermal_zone0/temp\"; for z in /sys/class/thermal/thermal_zone*; do type=$(cat $z/type 2>/dev/null); if [ \"$type\" = \"x86_pkg_temp\" ] || [ \"$type\" = \"TCPU\" ]; then temp_file=\"$z/temp\"; break; fi; done; cat \"$temp_file\" 2>/dev/null; fi"]
        running: false
        
        stdout: SplitParser {
            onRead: data => {
                const tempStr = data.trim()
                if (tempStr) {
