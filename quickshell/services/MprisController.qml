...
Singleton {
    id: root
    property MprisPlayer activePlayer: trackedPlayer ? trackedPlayer : (Mpris.players.values[0] ? Mpris.players.values[0] : null)
    ...
    property string trackTitle: activePlayer ? (activePlayer.trackTitle || "No media") : "No media"
    ...
    property string desktopEntry: activePlayer ? (activePlayer.desktopEntry || "") : ""
    ...
    function autoReevaluatePlayer() {
        ...
        for (let i = 0; i < players.length; i++) {
            ...
            let score = 0;
            if (player.playbackState === MprisPlaybackState.Playing) score += 100000;
            if (player.dbusName === root.lastPlayerDbusName) score += 10000;
            if (priorityIndex > -1) score += (priorityIndex + 1) * 1000;
        ...
        Connections {
            ...
            function onPlaybackStateChanged() {
                if (modelData.playbackState === MprisPlaybackState.Playing) {
                    root.lastPlayerDbusName = modelData.dbusName;
                }
