...
Item {
    ...
    readonly property bool isCharging: battery ? (battery.state === UPowerDevice.Charging || battery.state === 5) : false
    readonly property bool isFullyCharged: battery ? (battery.state === UPowerDevice.FullyCharged) : false
    readonly property bool isPluggedIn: isCharging || isFullyCharged
    readonly property bool isWarning: batteryLevel < 20 && batteryLevel >= 10
    readonly property bool isLow: batteryLevel < 10
    ...
    readonly property string timeString: {
        if (!battery) return ""
        const secs = isPluggedIn ? battery.timeToFull : battery.timeToEmpty
    ...
    property color compactBatteryColor: {
        ...
        if (batteryLevel < 10) return redColor;
        if (batteryLevel < 20) return yellowColor;
